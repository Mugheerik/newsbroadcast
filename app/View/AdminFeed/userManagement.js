...
const UserManagementScreen = () => {
  ...
  const toggleUserAccount = async (userUid, email, currentStatus) => {
    try {
      ...
      Alert.alert(`${action} User`, `Are you sure you want to ${action.toLowerCase()} the user: ${email}?`, [
        {
          text: 'Cancel',
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              const newStatus = currentStatus === 'disabled' ? 'user' : 'disabled'; // Toggle status
              ...
              Alert.alert('Error', `An error occurred while ${action.toLowerCase()}ing the user.`);
            }
          },
        },
      ]);
      ...
      <View style={styles.userCard}>
        <View style={styles.userInfoContainer}>
          ...
          <View style={styles.userInfo}>
            ...
            <Text style={[styles.userStatus, item.status === 'disabled' ? styles.disabledStatus : styles.activeStatus]}>
              Status: {item.status ? String(item.status) : 'N/A'}
            </Text>
          </View>
        </View>
        <TouchableOpacity
          style={[styles.button, item.status === 'disabled' ? styles.disabledButton : styles.activeButton]}
